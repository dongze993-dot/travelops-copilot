...
4. 以普通住宿、饮食、市内交通、游玩与少量伴手礼组成总费用区间，并单独展示公开资料页的费用原文；
5. 在计划末尾保留本次使用的资料链接，方便出发前自行确认。

填写的预算是总预算上限。页面按普通消费水平估算住宿、饮食、市内交通、游玩与少量伴手礼，不含往返目的地的大交通、购物大额消费和预订手续费；它不是实时报价。来源页中的票价会单独展示，同一原文里的通票和单票等可选价格不会相加。票价、营业时间、预约要求会变化，出行前应打开资料来源或官方渠道确认。
...
| Netlify 公开页面 | 静态前端 + Node.js Netlify Functions + 免费公开资料查询 | 面向游客的按日计划、来源链接和普通出行费用区间 |
