...
sequenceDiagram
    ...
    participant API as /api/v1/plans
...
3. **工作流可观察。** `workflow_trace` 的目的是让开发者和面试官看出请求经历了什么步骤；它不是生产级审计日志，也不应包含密钥或个人数据。
4. **外部系统通过适配层隔离。** 当前工单仅为模拟接口。将来接入 CRM 时，应替换适配层并补充鉴权、幂等、审批、审计和失败重试。
...
| LLM/网络服务不可用（若后续接入） | 使用受控降级或返回可解释错误 | 泄露密钥、隐藏失败或输出未经验证事实 |
