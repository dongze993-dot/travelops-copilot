...
7. **联网模式显式、最小化且可关闭。** v3 未配置 Key 时返回清楚的 `503`，不会伪装为联网；启用后仅向外部服务发送目的地和勾选偏好，并返回 URL、获取时间和缓存状态。函数的每 IP 限流、短时缓存和 Blobs 固定槽位日额度共同降低普通滥用；固定槽位是保守的演示可用性保护，不是事务型付费额度保证。
