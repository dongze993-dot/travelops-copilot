...
Netlify v3 另有一个免费公开资料查询适配层。它不改变上述 v1/v2 的合成基线：无需环境变量、Key、账号或付款，只读查询中文维基导游和中文维基百科。v3 返回短摘要、`https` 来源链接、获取时间和明确的核验提示；不会把资料摘要当作已验证的价格、营业状态或可订性。详细边界见 [免费公开资料说明](live-retrieval.md)。
...
| 免费公开资料适配层（Netlify v3） | 以 destination 查询中文维基导游与中文维基百科，清洗 URL/摘要并附获取时间 | 不接收备注、联系人或工单；不抓取全文、不作为实时价格/预订/地图事实 |
...
7. **免费公开资料模式最小化。** v3 不需要配置 Key；仅向中文维基导游和中文维基百科的只读接口发送目的地，并返回 URL、获取时间和缓存状态。每 IP 限流和短时缓存共同降低重复请求；公开资料覆盖不足时返回空结果，而不是伪装成全网实时搜索。
